...
- 心事 100 篇。
- 爆料 100 篇。
- 合计 300 篇，每篇约 470–520 个汉字。
- 每篇 24 条模拟讨论，其中包含缩进显示的评论回复。
- 标题在每个分类内保持 100 个不同组合。
- 执行前自动下载当前 CloudBase 内容 JSON 备份。
- 执行后整份替换 `gc_content.main` 中的 `posts`，保留首页、分类和其他站点配置。
- 前台会明确显示“故事创作 / 模拟讨论”说明，不把生成内容冒充真实用户经历或真实互动。
...
代码已经提交 GitHub，但云端文章是否已经被替换，必须以后台实际文章数量为准。

如果后台尚未出现“备份并替换为 300 篇长内容”按钮：
...
1. 登录后台。
2. 点击“备份并替换为 300 篇长内容”。
3. 确认操作。
4. 等待提示“已替换为 300 篇内容，并完成本地备份”。
5. 前台 Ctrl+F5，分别检查三个分类均为 100 篇。

本次没有修改 `cloudfunctions/gc-api/index.js` 或云函数依赖，因此不需要重新部署 `gc-api`。
